...
class MockMonitorStorage {
    ...
    async getAgentReportsByProject(projectId) {
        const reports = [];
    ...
    async getAgentReportsByCategory(category) {
        const reports = [];
        const allReports = Array.from(this.agentReports.values());
        for (const r of allReports) {
...
describe("Failure Detection", () => {
    ...
    describe("Security Gap Detection", () => {
        it.each(SECURITY_PATTERNS)("detects security pattern: %s", (pattern) => {
            ...
            const hasSecurityIssue = SECURITY_PATTERNS.some((p) => errorMessage.toLowerCase().includes(p.toLowerCase()));
    ...
    describe("Hallucination Detection", () => {
        it.each(HALLUCINATION_PATTERNS)("detects hallucination pattern: %s", (pattern) => {
            ...
            const hasHallucination = HALLUCINATION_PATTERNS.some((p) => errorMessage.toLowerCase().includes(p.toLowerCase()));
