...
describe("Validation", () => {
    ...
    describe("enforceValidation", () => {
        ...
        it("should enforce fake data check when strict and enabled", () => {
            const strictConfig = { ...mockConfig, validationLevel: "strict" };
            const fakeResponse = {
                ...strictValidResponse,
                recommendation: "lorem ipsum dolor sit amet is the solution for this comprehensive problem",
            };
            expect(() => enforceValidation(fakeResponse, strictConfig, { enforceFakeDataCheck: true })).toThrow(ValidationError);
        });
        it("should not throw for fake data when enforceFakeDataCheck is false and not strict", () => {
            const mediumConfig = { ...mockConfig, validationLevel: "medium" };
            const fakeResponse = {
                ...mockResponse,
                recommendation: "lorem ipsum dolor sit amet is the solution",
            };
            expect(() => enforceValidation(fakeResponse, mediumConfig, { enforceFakeDataCheck: false, enforceSecurity: false })).not.toThrow();
        });
    ...
    describe("enforceClassicThresholds", () => {
        ...
        it("should throw ValidationError for latency above threshold with descriptive message", () => {
            ...
            try {
                ...
                expect(error).toBeInstanceOf(ValidationError);
                expect(error.message).toContain("Latency");
                expect(error.message).toContain("exceeds threshold");
            }
        });
