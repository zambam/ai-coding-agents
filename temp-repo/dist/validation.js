...
export function checkFakeData(text) {
...
export function checkSecurity(response) {
...
export function validateResponse(response, validationLevel) {
    const passed = [];
    const failed = [];
...
export function enforceValidation(response, config, enforcement = {}, errorContext = {}) {
    ...
    if (isStrict && opts.enforceFakeDataCheck && fakeData.hasFakeData) {
        throw new ValidationError([`Fake/placeholder data detected: ${fakeData.patterns.join(", ")}`], errorContext);
    }
...
export function enforceClassicThresholds(agentType, metrics, errorContext = {}) {
