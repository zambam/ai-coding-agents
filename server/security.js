...
export function checkUnsafeCode(input) {
    const detectedPatterns = [];
...
export function checkPIIPatterns(input) {
    const detectedPatterns = [];
...
export function runSecurityChecks(input, options = {}) {
...
export function createSecurityMiddleware(options = {}) {
    ...
    return (req, res, next) => {
        ...
        req.securityEvents = result.events;
        next();
    };
}
...
function extractAllStrings(obj, maxDepth = 10, depth = 0) {
    if (depth > maxDepth)
        return [];
    if (typeof obj === "string") {
        return [obj];
...
export function runDeepSecurityChecks(input, options = {}) {
    const { blockOnDetection = true } = options;
    const events = [];
