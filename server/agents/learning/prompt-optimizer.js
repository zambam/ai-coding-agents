...
export class PromptOptimizer {
    ...
    async selectPromptVariant(agentType) {
    ...
    async createShadowVariant(agentType, promptText) {
    ...
    async promoteToABTest(variantId, trafficPercent = 10) {
        return this.storage.updatePromptVariant(variantId, {
            status: "ab_test",
            trafficPercent: Math.min(trafficPercent, 50),
        });
    }
    async computeVariantMetrics(variantId, outcomes) {
        const variantOutcomes = outcomes.filter((o) => o.promptVersion === variantId);
    ...
    async evaluateABTest(agentType) {
        const promoted = await this.storage.getPromotedVariant(agentType);
        ...
        for (const challenger of abVariants) {
            ...
            if (controlMetrics.sampleSize < this.config.minSampleSize ||
                challengerMetrics.sampleSize < this.config.minSampleSize) {
                continue;
            }
            const pValue = this.computePValue(controlMetrics.acceptanceRate, controlMetrics.sampleSize, challengerMetrics.acceptanceRate, challengerMetrics.sampleSize);
            const isSignificant = pValue < this.config.pValueThreshold;
            const improvement = challengerMetrics.acceptanceRate - controlMetrics.acceptanceRate;
    ...
    async promoteWinner(winnerId, loserId) {
