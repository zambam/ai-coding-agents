import type { AgentType, FeedbackTag } from "@shared/schema";
import type { IDataTelemetry, RunOutcome, HumanFeedback } from "../../storage";
...
export class OutcomeLearner {
  ...
  async analyzeAgentPerformance(agentType: AgentType): Promise<LearningSignal> {
    const outcomes = await this.storage.getRunOutcomesByAgent(
      agentType,
      500
    );
  ...
  async identifyFeedbackPatterns(agentType: AgentType): Promise<OutcomePattern[]> {
    const outcomes = await this.storage.getRunOutcomesByAgent(agentType, 200);
    const patterns: Map<string, OutcomePattern> = new Map();
  ...
  async generateImprovementSuggestions(agentType: AgentType): Promise<string[]> {
    const signal = await this.analyzeAgentPerformance(agentType);
    const patterns = await this.identifyFeedbackPatterns(agentType);

    const suggestions: string[] = [...signal.suggestedImprovements];

    const problemTags: FeedbackTag[] = ["too_verbose", "too_brief", "incorrect", "off_topic", "confusing"];
    
    for (const pattern of patterns) {
      if (problemTags.includes(pattern.pattern as FeedbackTag) && pattern.frequency >= 3) {
        suggestions.push(this.tagToSuggestion(pattern.pattern as FeedbackTag, pattern.frequency));
      }
    }
