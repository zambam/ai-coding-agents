...
import type { IDataTelemetry, MemoryEntry, InsertMemoryEntry } from "../../storage";
...
export class MemoryManager {
  ...
  async retrieveRelevantMemories(
    ...
    const embedding = this.generateSimpleEmbedding(taskDescription);
    const memories = await this.storage.findSimilarMemories(
      agentType,
      embedding,
    ...
    for (const memory of memories) {
      if (!memory.taskEmbedding) continue;
...
Response: ${m.entry.response.substring(0, 500)}${m.entry.response.length > 500 ? "..." : ""}`;
  ...
  async getMemoryStats(agentType: AgentType): Promise<{
    totalMemories: number;
    avgQualityScore: number;
    avgAccessCount: number;
  }> {
    const allMemories = await this.storage.findSimilarMemories(
      agentType,
