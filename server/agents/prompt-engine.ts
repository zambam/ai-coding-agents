...
import type { AgentConfig, ReasoningStep, ConsistencyMode } from "@shared/schema";
import type { ReasoningPath, ConsistencyResult, SelfCritiqueResult, PromptTemplate } from "./types";
...
export class PromptEngine {
  ...
  async generateWithCoT(
    systemPrompt: string,
    userPrompt: string,
    model: string = "gpt-4o"
  ): Promise<{ response: string; reasoning: ReasoningStep[]; tokensUsed: number }> {
  ...
  async runSelfConsistency(
    ...
    for (let i = 0; i < pathCount; i++) {
      const { response, reasoning } = await this.generateWithCoT(
        systemPrompt,
