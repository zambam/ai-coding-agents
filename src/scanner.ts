...
const FAKE_DATA_PATTERNS: PatternDefinition[] = [
  {
...
const SECURITY_PATTERNS: PatternDefinition[] = [
  {
...
export class FakeDataScanner {
  ...
  async scanContent(content: string, filename: string = "inline"): Promise<ScanIssue[]> {
    const issues: ScanIssue[] = [];
    const lines = content.split("\n");
...
export function createScanner(options?: Partial<ScannerOptions>): FakeDataScanner {
