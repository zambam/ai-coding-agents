...
export class ReplitMdParser {
  ...
  private parseContent(content: string): ReplitMdConfig {
    const config: ReplitMdConfig = this.getDefaultConfig();
    
    const consistencyMatch = content.match(/consistency\.mode:\s*(none|fast|robust)/i);
    if (consistencyMatch) {
      config.agentConfig.consistencyMode = consistencyMatch[1].toLowerCase() as any;
    }
    
    const validationMatch = content.match(/validationLevel:\s*(low|medium|high|strict)/i);
    if (validationMatch) {
      config.agentConfig.validationLevel = validationMatch[1].toLowerCase() as any;
    }
  ...
  private getDefaultConfig(): ReplitMdConfig {
    return {
      agentConfig: {},
      codeStandards: [],
      architecturalRules: [],
