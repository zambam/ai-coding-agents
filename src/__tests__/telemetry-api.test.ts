...
async function makeRequest(method: string, path: string, body?: unknown): Promise<{ status: number; body: unknown }> {
...
describe("Telemetry API Integration", () => {
  ...
  describe("Analytics API", () => {
    it("returns acceptance rate for agent type", async () => {
      ...
      expect((body as { acceptanceRate: number }).acceptanceRate).toBeCloseTo(0.75);
    });
    ...
    it("returns 0 acceptance rate for agent with no outcomes", async () => {
      ...
      expect(status).toBe(200);
      expect((body as { acceptanceRate: number }).acceptanceRate).toBe(0);
    });
  });
  ...
  describe("Zod Schema Validation", () => {
    it("validates classicMetrics structure in run outcome", async () => {
      ...
      expect((body as { classicMetrics: { cost: number } }).classicMetrics.cost).toBe(0.05);
    });
