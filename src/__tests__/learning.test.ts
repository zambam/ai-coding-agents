...
describe("Self-Learning Infrastructure", () => {
  ...
  describe("MemoryManager", () => {
    ...
    it("builds context from memories", async () => {
      ...
      const context = await memoryManager.buildContextFromMemories(
        "codeNinja",
        "Implement pagination for list endpoint"
      );
  ...
  describe("Integration: Learning Pipeline", () => {
    it("complete learning flow: store -> analyze -> improve", async () => {
      ...
      const variant = await optimizer.createShadowVariant(
        "architect",
        "You are The Architect, a system design expert..."
      );
      ...
      const metrics = await optimizer.computeVariantMetrics(
        variant.id,
        await storage.getRunOutcomesByAgent("architect")
      );
